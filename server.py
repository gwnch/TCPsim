...
while True:
    ...
    if inbound.Flag == 1:
        ...
        print("\nFlag: %d\nAcknowledgement Number: %d" % (outbound.Flag, outbound.Ack_Num))
    ...
    if inbound.Flag == 3:
        ...
        print("\nReceived New Data:")
        # append data to output final string
        STR = STR + inbound.Data
        print("Data:%s\nSequence Number: %d\nAcknowledgement Number: %d" % (inbound.Data, inbound.Seq_Num, inbound.Ack_Num))
    ...
    # beginnging closing connection
    if inbound.Flag == 4:
        print("\nReceived FIN.")
        print("\nFlag: %d\nAcknowledgement Number: %d\n" % (inbound.Flag, inbound.Ack_Num))

        print("Sending FIN ACK.")
        outbound.Data = inbound.Data
        outbound.Flag = inbound.Flag + 1
        outbound.Seq_Num = inbound.Seq_Num
        outbound.Ack_Num = inbound.Ack_Num + 1
        print("\nFlag: %d\nAcknowledgement Number: %d" % (outbound.Flag, outbound.Ack_Num))
